...
Box2D.b2Separator.Separate = 
function Separate(body, fixtureDef, verticesVec, scale, offset_x, offset_y) {
    /**
     ...
     * @param scale <code>[optional]</code> The scale which you use to draw shapes in Box2D. The bigger the scale, the better the precision. The default value is 1. 
     * @see b2PolygonShape
    ...
    for(i=0; i<n; i++) {
        verticesVec = new Array();
        vec = figsVec[i];
        m = vec.length;
        for(j=0; j<m; j++) {
            verticesVec.push(new b2Vec2(vec[j].x/scale+offset_x, vec[j].y/scale+offset_y));
        }
...
Box2D.b2Separator.Validate = 
function Validate(verticesVec) {
    /**
...
function calcShapes(verticesVec)
{
    ...
    var v, hitV;
    var isConvex;
    var figsVec = new Array(), queue = new Array();
...
function err()
{
    console.log("A problem has occurred. Use the Validate() method to see where the problem is.");
}
